...
{
    public class Zombie_JumpAttack : ZomBie_Attack
    {
        public float flySpd;
        ...
        public override void StateChange(ZombieState state)
        {
        }
        ...
        {
            var positio = FindFarPoint(zombieData.player.position, 10f, 20f);
            return positio;
        }
        ...
        public Vector3 FindFarPoint(Vector3 pivot, float minDistance = 6f, float maxDistance = 10f)
        {
            ...
            float angle = Random.Range(zombieData.player.rotation.eulerAngles.y * -1f, -zombieData.player.rotation.eulerAngles.y + 90);
